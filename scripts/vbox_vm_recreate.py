...
print("Creating VM...")
run("cloud-localds {seed_iso} {cloud_init}")
run("VBoxManage import {ova_file} --vsys 0 --vmname {vm_name}")
run("VBoxManage storageattach {vm_name} --storagectl IDE --port 1 --device 0 --type dvddrive --medium {seed_iso}")
run("VBoxManage modifyvm {vm_name} --cpus {cpu_count} --memory {ram_in_mb}")
run("VBoxManage startvm {vm_name} --type headless")
